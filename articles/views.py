...
from .forms import PostArticleForm, ArticleGenreSelectForm, ArticleRecordForm, ChatForm, ReplyForm
from .forms import all_article_genres
    ...
    if request.method == 'POST':
        ...
        if glist == []:
            ...
            for item in gps:
                glist.append(item.title)
        if genlist == ['-']:
            genlist = [item[0] for item in all_article_genres]
        articlerecord = get_your_group_articlerecord(user, glist, genlist, page)
    # GETアクセス時の処理
    else:
        # フォームの用意
        ...
        articlerecord = get_your_group_articlerecord(user, glist, genlist, page)
    # 共通
    genreform = ArticleGenreSelectForm()
    params={
        'login_user': user,
        'contents': articlerecord,
        'check_form': checkform,
        'gen_form': genreform,
        'onerror': onerror,
    }
    if request.user_agent.is_mobile:
        return render(request, 'articles_mobile_HTML/index.html', params)
    else:
        return render(request, "articles_HTML/index.html", params)
...
def post(request):
    ...
    if request.method == 'POST':
        # 送信内容の処理
        form = PostArticleForm(user, request.POST)
        ...
        if (form.is_valid()):
            ...
            # ArticleRecordを作成して保存
            prcd = ArticleRecord()
            prcd.owner = user
            prcd.group = group
            prcd.link = link
            prcd.title = title
            prcd.first_author = first_author
            ...
            prcd.report = report
            if link != '':
                prcd.rm_space() # 入力されたlinkからspaceを削除
            prcd.save()
            line_notification(user, group, att="post", url=f"/articles/chat/{prcd.id}", record=prcd) # LINEに通知
            print("ArticleRecordポスト成功")
            # メッセージを設定
    ...
    if request.user_agent.is_mobile:
        return render(request, 'articles_mobile_HTML/post.html', params)
    else:
        return render(request, "articles_HTML/post.html", params)


# chatボタンの処理
@login_required(login_url='/')
def chat_button(request, chat_id):
    # chatするArticleRecordを取得
    chat_prcd = ArticleRecord.objects.get(id=chat_id)
    messages.success(request, 'Chatページに移行します！')
    return redirect(to="/articles/chat.html")
...
def chat(request, chat_id):
    # login中のuserを取り出す
    user = Account.objects.get(user=request.user)
    chat_theme = ArticleRecord.objects.get(id=chat_id)
    ...
    if request.method == 'POST':
        ...
        chat.save()
        # ArticleRecordのchat_countを1増やす
        # num = Chat.objects.filter(Articlerecord=chat_theme).count()
        chat_theme.chat_count += 1
        # chat_theme.chat_count = num
        chat_theme.save()
        line_notification(user, chat_theme.group, att="comment", url=f"/articles/chat/{chat_id}", record=chat_theme) # LINEに通知
        print("POST OK")
    ...
    # GETアクセス時の処理
        ...
        pass
    # 共通処理
    user.get_history(app="Articles", record=chat_theme)
    user.save()
    form = ChatForm()
    chat = ArticleChat.objects.filter(record=chat_theme).filter(reply_id__lt = 0)
    reply = ArticleChat.objects.filter(record=chat_theme).filter(reply_id__gt = 0)
    params = {
        'login_user': user,
        ...
        'form': form,
        'id': chat_id,
        'img_path': chat_theme.link,
        'onerror': onerror,
    }
    if request.user_agent.is_mobile:
        return render(request, 'articles_mobile_HTML/chat.html', params)
    else:
        return render(request, 'articles_HTML/chat.html', params)
...
def highlight(request, highlight_id):
    # login中のuserを取り出す
    user = Account.objects.get(user=request.user)
    chat_theme = ArticleRecord.objects.get(id=highlight_id)
    ...
    if spaceless == "" or spaceless == None:
        ...
        return redirect(to=f'/articles/chat/{highlight_id}')
    # Chatを作成して保存
    chat = ArticleChat()
    chat.owner = user
    chat.record = chat_theme
    ...
    # messages.success(request, "コメントしました。")
    insert_tag(highlight_id, chat_theme, comment, chat.id, app="articles") # ユーザーが選択した文字列をリンクにする
    
    # return redirect(to=f"/books/chat/{highlight_id}/reply/{reply_id}")
    return redirect(to=f'/articles/chat/{highlight_id}/reply/{chat.id}')
...
def reply(request, chat_id, reply_id):
    ...
    user = Account.objects.get(user=request.user)
    # リプライするチャットを取得
    chat = ArticleChat.objects.get(id=reply_id)
    # チャットテーマとなるレコードを取得
    chat_theme = chat.record
    # チャットへのリプライをすべて取得
    replies = ArticleChat.objects.filter(record=chat.record).filter(reply_id__gt = 0).filter(reply_id=chat.id)
    # 閲覧権があるかチェック なければTOPへ戻る
    group = chat_theme.group
    if group.title == 'public':
        pass
    ...
    if request.method == 'POST':
        ...
        chat.reply_count += 1
        chat.save()
        line_notification(user, chat_theme.group, att="comment", url=f"/articles/chat/{chat_id}/reply/{reply_id}", record=chat_theme) # LINEに通知
        # else:
            # messages.error(request, "validation error occured.")
    # GET
    else:
    ...
    if request.user_agent.is_mobile:
        return render(request, 'articles_mobile_HTML/reply.html', params)
    else:
        return render(request, "articles_HTML/reply.html", params)
...
def edit(request, edit_id):
    # login中のuserを取り出す
    user = Account.objects.get(user=request.user)
    prcd = ArticleRecord.objects.get(id=edit_id)
    pre_list = [prcd.group, prcd.genre, prcd.score, prcd.summary, prcd.report]
    if prcd.owner != user:
        messages.success(request, '自分のでないレコードは編集できません。')
        return redirect(to=f'/articles/chat/{edit_id}')
    # POST送信時の処理
    if (request.method == 'POST'):
        print("POST!!!")
        reviced = ArticleRecordForm(request.POST, instance=prcd)
        group_form = GroupSelectForm(user, request.POST)
        rev_group = request.POST.get("groups")
        genre_form = ArticleGenreSelectForm(request.POST)
        ...
        if (reviced.is_valid() and group_form.is_valid() and genre_form.is_valid()):
            ...
            prcd.save()
            reviced.save()
            post_list = [prcd.group, prcd.genre, prcd.score, prcd.summary, prcd.report]
            if post_list == pre_list: # まったく編集せずPOST!をクリックした場合
                return redirect(to=f'/articles/chat/{edit_id}') # edit_dateは更新されてしまうが、edit_countは更新されない
            prcd.edit_count += 1
            prcd.save()
            return redirect(to=f'/articles/chat/{edit_id}')
    ...
    # GETアクセス時の処理
        ...
        reviced = ArticleRecordForm(instance=prcd)
        genre_form = ArticleGenreSelectForm({"genre": prcd.genre})
    ...
    if request.user_agent.is_mobile:
        return render(request, 'articles_mobile_HTML/edit.html', params)
    else:
        return render(request, 'articles_HTML/edit.html', params)
...
def repost(request, repost_id):
    # login中のuserを取り出す
    user = Account.objects.get(user=request.user)
    brcd = ArticleRecord.objects.get(id=repost_id)
    if brcd.owner != user:
        messages.success(request, '自分のでないレコードはリポストできません。')
        return redirect(to=f'/articles/chat/{repost_id}')
    # POST送信時の処理
    if (request.method == 'POST'):
        repost = ArticleRecord()
        reviced = ArticleRecordForm(request.POST, instance=brcd)
        group_form = GroupSelectForm(user, request.POST)
        rev_group = request.POST.get("groups")
        genre_form = ArticleGenreSelectForm(request.POST)
        rev_genre = request.POST.get("genre")
        ...
        if (reviced.is_valid() and group_form.is_valid() and genre_form.is_valid()):
            ...
            if repost.group == brcd.group:
                ...
                return redirect(to=f'/articles/repost/{repost_id}')
            repost.save()
    ...
    # GETアクセス時の処理
        ...
        reviced = ArticleRecordForm(instance=brcd)
        genre_form = ArticleGenreSelectForm({"genre": brcd.genre})
    ...
    if request.user_agent.is_mobile:
        return render(request, 'articles_mobile_HTML/repost.html', params)
    else:
        return render(request, 'articles_HTML/repost.html', params)
...
def delete(request, del_id):
    # login中のuserを取り出す
    user = Account.objects.get(user=request.user)
    prcd = ArticleRecord.objects.get(id=del_id)
    if prcd.owner != user:
        messages.success(request, "自分のでないレコードは削除できません")
        return redirect(to=f'/articles/chat/{del_id}')        
    # POST送信時の処理
    if (request.method == 'POST'):
        prcd.delete()
        chats = ArticleChat.objects.filter(record=prcd)
        for chat in chats:
            chat.delete()
    ...
    if request.user_agent.is_mobile:
        return render(request, 'articles_mobile_HTML/delete.html', params)
    else:
        return render(request, 'articles_HTML/delete.html', params)

# 以降は普通の関数=====================================================================

def get_your_group_articlerecord(owner, glist, genlist, page):
    page_num = 8 # ページ当たりの表示数
    # publicの取得
    (public_user, public_group) = get_public()
    # チェックされたGroupの取得
    groups = Group.objects.filter(title__in=glist)
    # Groupがgroupsかme_groupsに含まれるArticleRecordの取得
    articlerecords = ArticleRecord.objects.filter(group__in=groups).filter(genre__in=genlist).distinct()
    # ページネーションで指定ページを取得
    page_item = Paginator(articlerecords, page_num)
